...
import { FaHome, FaChartLine, FaDollarSign, FaUniversity, FaUser, FaPlus } from "react-icons/fa";
    ...
    <div className="flex h-screen bg-gray-900 text-white">
      ...
      <aside className="w-64 bg-gray-800 p-6 flex flex-col">
        ...
        <nav className="flex flex-col space-y-4">
          {[
            { tab: "home", icon: <FaHome />, text: "Home", path: "/dashboard/loggedin" },
            { tab: "chart", icon: <FaChartLine />, text: "Chart", path: "/chart/loggedin" },
            { tab: "asset", icon: <FaDollarSign />, text: "Asset", path: "/asset/loggedin" },
            { tab: "finance", icon: <FaUniversity />, text: "Finance", path: "/finance/loggedin" },
        ...
        <section className="mb-8">
          ...
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
            <table className="w-full text-left">
              ...
              <tbody>
                {[
                  { type: "Ethereum Purchased", amount: "0.0154 ETH", total: "$10.00", status: "Pending", date: "Feb 21, 2021" },
                  { type: "Bitcoin Purchased", amount: "0.3 BTC", total: "$10.00", status: "Done", date: "Feb 14, 2021" },
                  { type: "Bitcoin Purchased", amount: "0.025 BTC", total: "$10.00", status: "Done", date: "Jan 14, 2021" },
                ].map(({ type, amount, total, status, date }, index) => (
                  <tr key={index} className="border-t border-gray-700">
                    <td className="py-3">{type}</td>
                    <td className="py-3">{amount}</td>
                    <td className="py-3">{total}</td>
                    <td className={`py-3 ${status === "Done" ? "text-green-400" : "text-yellow-400"}`}>{status}</td>
