    ...
    try {
      ...
      if (!Number.isNaN(numericAmount) && numericAmount >= 100) {
        ...
        const res = await fetch('/api/sendOTP', {
          ...
          body: JSON.stringify({ email: user.email }), // ใช้อีเมลจาก user context
        });

        const data = await res.json();
        if (data.success) {
          console.log("Sent OTP:", data.otp); // Debug: ตรวจสอบว่า OTP ส่งถูกต้อง
          router.push(`/finance/deposit?amount=${amount}&sentCode=${data.otp}`); // ส่ง OTP และจำนวนเงินไปใน query
        } else {
